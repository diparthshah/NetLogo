...
public final strictfp class _anyturtleson extends Reporter {
  ...
  public Boolean report(final org.nlogo.nvm.Context context) throws LogoException {
    ...
    if (agentOrSet instanceof Turtle) {
      ...
      //return ((Patch) agentOrSet).size() > 0;
      if (((Patch) agentOrSet).size() > 0) {
      ...
      if (sourceSet.kind() == AgentKindJ.Turtle()) {
        for (AgentIterator iter = sourceSet.iterator(); iter.hasNext();) {
          ...
          //return ((Turtle) iter.next()).size() > 0;
          if (((Turtle) iter.next()).size() > 0) {
        ...
        for (AgentIterator iter = sourceSet.iterator(); iter.hasNext();) {
          ...
          //return ((Patch) iter.next()).size() > 0;
          if (((Patch) iter.next()).size() > 0) {
